...
//    spreadsheetId: '10QHfEDwmF3N9E1KBnC7XP_DW-Ht51j5UmkRuH-KGbYQ',
  ...
  }).then(function (response)
  {
    ...
    {
      var dept_name = 'unknown';
      for (var i = 1; i < sheets.departments.values.length; i++)
      {
        if (depts[dept] === sheets.departments.values[i][sheets.departments.cols.acad_org])
        {
          dept_name = sheets.departments.values[i][sheets.departments.cols.formaldesc];
          break;
        }
      }
      state_info += '<tr><td>' + dept_name + '</td><td>' +
                    docs_by_dept[depts[dept]].num_docs + '</td>' +
      '<td>' + docs_by_dept[depts[dept]].num_by_type + '</td>' +
      '<td>' + docs_by_dept[depts[dept]].num_by_prog + '</td></tr>';
    }

    state_info += '<tr class="totals"><td>Total</td><td>' + total_docs + '</td><td>' + total_by_type +
    ...
    for (var row = 1; row < sheets.editors.values.length; row++)
    {
      ...
      {
        console.log('User', sheets.editors.values[row]);
        user_found = true;
