...
# -------------------------------------------------------------------------------------------------
def send_email(
    from_addr, to_addrs, subject, html_body, text_body, cc_addrs=[], bcc_addrs=[], reply_to=None
):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Commandline interface to sendemail.py", add_help=False
    )
    parser.add_argument("-?", "--help", action="help")
    parser.add_argument(
        "-s", "--subject", default=f"Test Message {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
