...
def format_course(course, active_only=False):
    ...
    transfer_link = (
        f"https://explorer.cuny.edu/course-transfer/{course.course_id:06}/{course.offer_nbr}"
    )
    ...
    if course.course_id in cross_listed:
        ...
        # But it would be an error for career, requisites, description, designation, etc. to vary,
        # so we assume they don’t. (There are two known cases of career errors, which OUR is
        # correcting as we speak. There are no observed  errors of the other types.)
        # There is no way to get a different attributes list, because those depend only on
        # course_id.
    ...
    if not active_only:
        if course.course_status != "A":
            note = """
            <div class="warning">
              <strong>Note:</strong> Course is not active in CUNYfirst
            </div>"""
...
def course_search(search_str, include_inactive=False, debug=False):
    """Parse search string to get institution, discipline, and catalog_number, then find all
    matching courses and return an array of catalog entries.
