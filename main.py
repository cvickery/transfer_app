...
from course_requirements import lookup_course
...
def course_requirements():
  ...
  if institution and discipline and catalog_nbr:
    try:
      for course_dict in lookup_course(institution, discipline, catalog_nbr):
        print(course_dict)
        result += f"""
        <section>
          <h2>
            {course_dict['institution']}
            {course_dict['discipline']}
            {course_dict['catalog_number']}:
            <em>{course_dict['title']}</em>
          </h2>
        </section>
        """
