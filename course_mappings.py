...
""")
program_dict = {
    (row.institution, row.requirement_id): ProgramInfo._make(
        [
            row.block_type,
            row.block_value,
...
# -------------------------------------------------------------------------------------------------
def _to_html(
    institution: str, discipline: str, course_dict: dict, program_types, count_limit
) -> str:
    """ """
        ...
        if (skipped_majors + skipped_minors + skipped_concentrations + skipped_limit) > 0:
            ...
            for situation in situations.keys():
                if situations[situation][0]:  # number skipped
                    body += f"""<p>
                                  Skipped {situations[situation][0]}
                                  {situation}{situations[situation][1]}
                                </p>"""
...
def course_mappings_impl(request):
    ...
    header_str = header(
        title="Requirements by Course",
        nav_items=[
            {"type": "link", "text": "Main Menu", "href": "/"},
            {"type": "link", "text": "Programs", "href": "/registered_programs"},
        ],
    )
    result = f"{header_str}"
    result += """
  ...
  """
    ...
    if discipline:
        if catalog_number:
            ...
            catalog_choices = (
                f"<details><summary>Select Course ({discipline} {catalog_number})</summary><hr>"
            )
        else:
    ...
    if course_dict:
        course_mapping_html = _to_html(
            institution, discipline, course_dict, program_types, count_limit
        )
    else:
