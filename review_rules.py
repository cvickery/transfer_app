...
def do_form_0(request, session):
  ...
  """
    ...
    {
        header(
            title="Review Rules: Select Colleges",
    ...
    """
    response = render_template("review_rules.html", title="Select Colleges", result=Markup(result))
...
def do_form_1(request, session):
    ...
    with psycopg.connect("dbname=cuny_curriculum") as conn:
        with conn.cursor(row_fctory=namedtuple_row) as cursor:
            ...
            if len(session["source_institutions"]) == 1:
                ...
                criterion = (
                    "the sending college is " + institution_names[session["source_institutions"][0]]
                )
                sending_heading = (
                    f"{institution_names[session['source_institutions'][0]]}’s".replace("s’s", "s’")
                )
            if len(session["destination_institutions"]) == 1:
                receiving_is_singleton = True
                receiving_heading = (
                    f"{institution_names[session['destination_institutions'][0]]}’s".replace(
                        "s’s", "s’"
                    )
                )
    ...
    for cuny_subject in cuny_subjects:
        ...
        #   Both the college and discipline names will be displayed for each cuny_subject, unless
        #   there is only one college involved ("singleton"), in which case only the discipline name
        #   is shown.
        ...
        if sending_is_singleton:
            if len(source_disciplines_set) > 1:
                source_disciplines_str = (
                    "<div>" + "</div><div>".join(source_disciplines_set) + "</div>"
                )
            else:
        ...
        if receiving_is_singleton:
            ...
            if len(destination_disciplines_set) > 1:
                destination_disciplines_str = (
                    "<div>" + "</div><div>".join(destination_disciplines_set) + "</div>"
                )
            else:
            ...
            for discipline in destination_disciplines_set:
                ...
                colleges[discipline[0]].append(discipline[1])
            for college in colleges:
                destination_disciplines_str += "<div>{}: <em>{}</em></div>".format(
                    institution_names[college], ", ".join(colleges[college])
                )
        ...
        if source_disciplines_str != "":
            ...
            source_label = (
                f'<label for="source-subject-{cuny_subject}">{source_disciplines_str}</label>'
            )
            source_cbox = f"""
  ...
  {
        header(
            title="Review Rules: Select Disciplines",
            nav_items=[
                {"type": "link", "href": "/", "text": "Main Menu"},
                {"type": "link", "href": "/review_rules", "text": "Change Colleges"},
            ],
        )
    }
  <details open>
...
def do_form_2(request, session):
    ...
    with psycopg.connect("dbname=cuny_curriculum") as conn:
        with conn.cursor(row_fctory=namedtuple_row) as cursor:
            # Look up transfer rules where the sending course belongs to a sending institution and
            # is one of the source disciplines and the receiving course belongs to a receiving
            # institution and is one of the receiving disciplines.
            try:
                source_institution_params = ", ".join("%s" for i in session["source_institutions"])
                destination_institution_params = ", ".join(
                    "%s" for i in session["destination_institutions"]
            ...
            if len(source_subject_list) < 1:
                return render_template(
                    "review_rules.html",
                    result=Markup('<h1 class="error">No sending disciplines selected.</h1>'),
                )
            if len(destination_subject_list) < 1:
                return render_template(
                    "review_rules.html",
                    result=Markup('<h1 class="error">No receiving disciplines selected.</h1>'),
                )

            # Prepare the query to get the set of rules that match the institutions and
            # cuny_subjects selected.
      ...
      order by source_institution, destination_institution, subject_area, group_number"""
            cursor.execute(
                q, (session["source_institutions"] + session["destination_institutions"])
            )

            if cursor.rowcount < 1:
                return render_template(
                    "review_rules.html",
                    result=Markup('<h1 class="error">There are no matching rules.</h1>'),
                )

            all_rules = cursor.fetchall()
            selected_rules = []
            # Get the source and destination course lists from the above set of rules where the
            # destination subject was selected. It's possible to have selected rules that don’t
            # transfer to any of the selected destination subjects, so those rules are dropped while
            # building the selected-rules list.
            if request.form.get("all-destination-subjects"):
                destination_subjects_clause = ""
            else:
                # Create a clause that makes sure the destination course has one of the destination
                # subjects
                destination_subject_list = request.form.getlist("destination_subject")
                destination_subject_params = ", ".join(f"'{s}'" for s in destination_subject_list)
                destination_subjects_clause = (
                    f" and dc.cuny_subject in ({destination_subject_params})"
                )

            for rule in all_rules:
                # It’s possible some of the selected rules don’t have destination courses in any of
                # the selected disciplines, so that has to be checked first.
                cursor.execute(
                    f"""
        select  dc.course_id,
                dc.offer_nbr,
          ...
          """,
            ...
            """,
                    ...
                    # Create the Transfer_Rule tuple suitable for passing to format_rules, and add
                    # it to the list of rules to pass.
  ...
  {
        header(
            title="Review Rules: Review Selected Rules",
            nav_items=[
                {"type": "link", "href": "/", "text": "Main Menu"},
                {"type": "link", "href": "/review_rules", "text": "Change Colleges"},
                {"type": "button", "class": "back-button", "text": "Change Subjects"},
            ],
        )
    }
    <details open>
...
def do_form_3(request, session):
    ...
    if len(kept_reviews) == 0:
      ...
      {
                header(
                    title="Review Rules: Respond to Email",
                    nav_items=[
                        {"type": "link", "href": "/", "text": "Main Menu"},
                        {"type": "link", "href": "/review_rules", "text": "Review More Rules"},
                    ],
                )
            }
      <details>
