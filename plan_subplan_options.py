    ...
    """)
        ...
        for row in cursor:
            ...
            if (this_institution := institution[0:3]) != last_institution:
                options_dict[this_institution] = {"options_list": [], "requirement_ids": []}
                if last_institution is not None:
                    options_dict[last_institution]["options_html"] = "\n".join(
                        options_dict[last_institution]["options_list"]
                    )
            ...
            if (block_type := row.block_type) != last_block_type:
                ...
                options_dict[this_institution]["options_list"].append(
                    f'<optgroup label="{nice_name}">'
                )
