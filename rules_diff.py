...
SourceCourses = namedtuple(
    "SourceCourses",
    "rule_key course_id offer_nbr  min_credits max_credits credits_source min_gpa max_gpa",
)
DestinationCourses = namedtuple(
    "DestinationCourses", "rule_key course_id offer_nbr  transfer_credits"
)
...
def diff_rules(first, second, debug=False, rogues=False):
    ...
    if debug:
        print(
            f"{len(first_keys):,} {first_date} rules and {len(second_keys):,} {second_date} rules "
            f"=>  {len(all_archive_keys):,} rules to check",
            file=sys.stderr,
        )
        ...
        try:
            ...
            if debug:
                if first_rules_source[key] != second_rules_source[key]:
                    print(
                        f"SSSD {key}: {first_rules_source[key]} :: {second_rules_source[key]}",
                        file=sys.stderr,
                    )
            ...
            if (
                first_rules_source[key] == second_rules_source[key]
                and first_rules_destination[key] == second_rules_destination[key]
            ):
                pass
            else:
                result[key] = {
                    first_date: (first_rules_source[key], first_rules_destination[key]),
                    second_date: (second_rules_source[key], second_rules_destination[key]),
                }
                if rogues:
                    # Rogue detector: if new rule is one course and it's blanket credit, report it
                    # to the proper authorities.
                    if (
                        len(first_rules_destination[key]) == 1
                        and len(second_rules_destination[key]) == 1
                        and not is_bkcr(first_rules_destination[key][0])
                        and is_bkcr(second_rules_destination[key][0])
                    ):
...
if __name__ == "__main__":
    ...
    first_date, second_date, result = diff_rules(
        dates[0], dates[1], debug=args.debug, rogues=args.rogues
    )
