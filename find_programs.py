...
with psycopg.connect("dbname=cuny_curriculum") as conn:
    ...
    """)
        ...
        for row in cursor.fetchall():
            if row.title is None:
                ...
                plans[row.cip_code].add(
                    (row.institution, row.plan, row.enrollment, row.description)
                )
            else:
    ...
    """)
        for row in cursor.fetchall():
            ...
            cip_words = tuple(
                stemmer.stem(word)
                for word in cip_string.lower().split()
            ...
            soc_words = tuple(
                stemmer.stem(word)
                for word in soc_string.lower().split()
                if len(word) >= min_word_length
            )
        ...
        # Cache the names of the coarse, medium, and fine CIP codes (i.e., all of them: the
        # cip2020codes table is already organized with separate 2, 4, and 6 digit codes)
        cursor.execute("""
...
def find_programs(search_request: dict):
    ...
    # Select cip_codes where the portion of search_words appearing in cip_soc_words is “large
    # enough.”
    ...
    for cip_code, cip_soc_str in cip_soc_strings.items():
        ...
        if large_enough == 0.0 or (
            num_search_words > 0 and ((num_matches / num_search_words) >= large_enough)
        ):
    ...
    return_dict = {
        "coarse": {"cip_codes": list(sorted(coarse_cip_set)), "plans": list(coarse_plan_set)},
        "medium": {"cip_codes": list(sorted(medium_cip_set)), "plans": list(medium_plan_set)},
        "fine": {"cip_codes": list(sorted(fine_cip_set)), "plans": list(fine_plan_set)},
    }
